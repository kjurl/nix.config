{ pkgs, stdenv, lib, fetchurl, mkWindowsApp, makeDesktopItem, imagemagick
, copyDesktopIcons, copyDesktopItems, winetricks }:
let
  version = "v7.6.9";
  ...
  srcs = {
    fusion360-installer = fetchurl {
      url =
        "https://github.com/cryinkfly/Autodesk-Fusion-360-for-Linux/blob/0df56158500bc1c8bb19c209e14747fef76b411f/files/builds/stable-branch/bin/install.sh";
...
in mkWindowsApp rec {
  ...
  runtimeInputs = with pkgs; [ wineWowPackages.waylandFull winetricks yad ];
  ...
  desktopItems = let
    ...
    mimeTypes = [ "x-world/x-3dmf" "application/x-3dmf" ];
  in [
    (makeDesktopItem {
      inherit mimeTypes;
