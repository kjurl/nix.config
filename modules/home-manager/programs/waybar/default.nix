{ lib, pkgs, config, ... }:
let
  kys = lib.utils.findKys ./.;
  flavours = import ./flavours.nix { inherit pkgs; };
in {
  options = lib.utils.setOptions kys {
    enable = lib.mkEnableOption "waybar";
    flavour = lib.mkOption {
      type = lib.types.enum lib.attrNames flavours;
      default = "default";
    };
  };
  ...
  in lib.mkIf cfg.enable {
    ...
    # desktop.hyprland.settings = mkIf (cfg.flavour == "win10") {
    ...
    # (builtins.toString ../../../config/waybar);
    # xdg.configFile."waybar".source = flavours."${cfg.flavour}";
  };
}
