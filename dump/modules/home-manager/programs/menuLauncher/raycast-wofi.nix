{ lib, pkgs, config, ... }:
let
  kys = lib.u.m.findKys ./. ++ [ "wofi" ];
  styles = {
    "0" = {
      # https://github.com/calthejuggler/awesome-wofi/tree/main/themes/raycast
      url =
    ...
    "1" = {
      # https://gist.github.com/veloii/033300e532c43e3cdbd25a145bae2c66
      url =
...
in {
  options = lib.u.m.setOptions kys {
  ...
  config = let
    ...
    cfg = lib.u.m.getConfig config kys;
